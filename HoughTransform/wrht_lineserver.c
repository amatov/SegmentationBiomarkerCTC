 ...
 *
 * File:    	wrht_lineserver.c
 * Purpose: 	resolving options for the Window Randomized Hough Transform (WRHT) 
                including the Connective andomized Hough Transform (CRHT)
 * Date:    	Jun 1, 1993
...
char *argv[];
{
  ...
  char *cptr, File[256], GrayFile[256], ResultFile[256], RealParamsFile[256],
	ParametersOut[256];
  int c, dim_x, dim_y, gray_dim_x, gray_dim_y, TextInfo=0, Shrink=0,
      Threshold=2, NumOfTests=1, NumOfMaxs=999, ThreshForPoints=0,
      WinSize=7, DontRmAccu=0, ConnectiveCheck=0, Sectoring=0,
      MinSegLen=10, MaxSegWidth=2, MaxSegGap=5, FileFormat=PGM_PIC,
      StoreMaximas=0, PostScript=0;
  double Accuracy=0.01, TolForFitting=0.25, Noise=0.0;

  File[0]=GrayFile[0]=ResultFile[0]=RealParamsFile[0]='\0';
  cptr=rindex(argv[0],'/');
  strcpy(proc_name,cptr?++cptr:argv[0]);

  while ((c = getopt(argc,argv,"f:G:R:r:F:sn:l:w:g:m:Mihc:t:W:T:E:DCSa:puP:")) != -1)
    switch (c) {
  ...
  if (make_object_from_image(File,pic1,Shrink,&dim_x,&dim_y,Noise)) {
    ...
    if (ThreshForPoints==0)
      ThreshForPoints=(int)((3.0*WinSize)/2.0)<MinSegLen?
                      (int)((3.0*WinSize)/2.0):MinSegLen;
...
void show_usage()
{
  ...
  fprintf(stderr,"[-c NumOfTests] [-t Threshold] [-W WinSize] ");
  fprintf(stderr,"[-T ThreshForPoints] [-E ToleranceErr] [-D] [-C] [-S]");
  fprintf(stderr,"[-a Accuracy] [-p]\n");
}

/*****************************************************************************/
void print_options()
{
  fprintf(stdout,"Window Randomized Hough Transform (WRHT) for lines \n");
  fprintf(stdout," including Connective Randomized Hough Transform (CRHT).\n\n");
  ...
  fprintf(stdout,"\t-W WinSize\t\t\t\t\tdef: 7\n");
  fprintf(stdout,"\t   The size of the window ((Msize*2+1)^2).\n");
  fprintf(stdout,"\t-T ThresholdForPoints\t\t\t\tdef: MIN(3*Msize/2,MinSegLen)\n");
  fprintf(stdout,"\t   Threshold for the number of points in the window.\n");
  fprintf(stdout,"\t-E TolForFitting\t\t\t\tdef: 0.25\n");
  fprintf(stdout,"\t   Maximum mean square error for curve fitting.\n");
  fprintf(stdout,"\t-D\t\t\t\t\t\tdef: not in use\n");
  fprintf(stdout,"\t   Do not remove the accumulator when line found, instead \n");
  fprintf(stdout,"\t   remove only one cell from the accumulator.\n");
  fprintf(stdout,"\t-C\t\t\t\t\t\tdef: not in use\n");
  fprintf(stdout,"\t   Use only points which are connected to center point \n");
  fprintf(stdout,"\t   of the window. If set, the method is the CRHT.\n");
  fprintf(stdout,"\t-S\t\t\t\t\t\tdef: not in use\n");
  fprintf(stdout,"\t   Do the connective component search as sectored.\n");
