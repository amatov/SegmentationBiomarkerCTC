...
if GUI
    ...
    %PSMA.origIm = PSMA.origIm(1000:3700,1000:3700);
    PSMA.im = PSMA.origIm;
    ...
    %DAPI.origIm = DAPI.origIm(1000:3700,1000:3700);
    DAPI.im = DAPI.origIm;
    ...
    %CD45.origIm = CD45.origIm(1000:3700,1000:3700);
    CD45.im = CD45.origIm;
end
...
parfor i = 1:partsI
    for j = 1:partsJ
        [~,PSMAwavelet{i,j}] = spotDetector(double(PSMA.im(max(1,waveletSteps*(i-1)-waveletBorder+1):min(waveletSteps*i+waveletBorder,size(PSMA.im,1)),max(1,waveletSteps*(j-1)-waveletBorder+1):min(waveletSteps*j+waveletBorder,size(PSMA.im,2)))));
        ...
        [~,DAPIwavelet{i,j}] = spotDetector(double(DAPI.im(max(1,waveletSteps*(i-1)-waveletBorder+1):min(waveletSteps*i+waveletBorder,size(DAPI.im,1)),max(1,waveletSteps*(j-1)-waveletBorder+1):min(waveletSteps*j+waveletBorder,size(DAPI.im,2)))));
        ...
        [~,CD45wavelet{i,j}] = spotDetector(double(CD45.im(max(1,waveletSteps*(i-1)-waveletBorder+1):min(waveletSteps*i+waveletBorder,size(CD45.im,1)),max(1,waveletSteps*(j-1)-waveletBorder+1):min(waveletSteps*j+waveletBorder,size(CD45.im,2)))));
...
imToProcess = PSMA.im;
sorted = sort(imToProcess(:));
p = round(997*length(imToProcess(:))/1000);
imToProcess(imToProcess>sorted(p)) = sorted(p);
p = round(3*length(imToProcess(:))/1000);
imToProcess(imToProcess<sorted(p)) = sorted(p);
imToProcess(imToProcess<0) = 0;

imToProcess = imToProcess - min(imToProcess(:));
imToProcess = imToProcess / max(imToProcess(:))*256;


PSMA.activeCont = activecontour(imToProcess,PSMA.wavelet);
...
disp('Careful! Here I am using DAPI instead of PSMA in the next line!');
keyboard;
disp('Careful! Here I am using DAPI instead of PSMA in the next line!');
keyboard;
PSMA.borderIm(DAPI.waveletWatershed == 0) = 1;
