 ...
 *
 * File:    	sht_lineserver.c
 * Purpose: 	resolving options for the Standard Hough Transform (SHT)
 * Date:    	Jun 1, 1993
...
char *argv[];
{
  ...
  char *cptr, File[256], GrayFile[256], ResultFile[256], RealParamsFile[256],
	ParametersOut[256];
  pic_type Threshold=10;
  int c, dim_x, dim_y, dim_rho=ACC_MAX_SIZE, dim_theta=ACC_MAX_SIZE,
      gray_dim_x, gray_dim_y, TextInfo=0, Shrink=0, NumOfMaxs=999, MinSegLen=10,
  ...
  while ((c = getopt(argc, argv, "f:G:R:r:F:sn:l:w:g:m:Mihd:t:uP:")) != -1)
    switch (c) {
  ...
  if (make_object_from_image(File,pic1,Shrink,&dim_x,&dim_y,Noise)) {
    ...
    if (strlen(GrayFile)) {
      ...
      if (dim_x!=gray_dim_x || dim_y!=gray_dim_y)
        warn_prnt("Edge and graylevel images are not equally sized.\n");
...
void show_usage()
{
  ...
  fprintf(stderr,"[-d dim_rho dim_theta] [-t Threshold]\n");
}

/*****************************************************************************/
void print_options()
{
  fprintf(stdout,"Standard Hough Transform (SHT) for lines.\n\n");
  ...
  fprintf(stdout,"\t   Accumulator array size (max %d * %d).\n",
          ACC_MAX_SIZE,ACC_MAX_SIZE);
  fprintf(stdout,"\t-t Threshold\t\t\t\t\tdef: 10\n");
