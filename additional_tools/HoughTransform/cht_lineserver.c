 ...
 *
 * File:         cht_lineserver.c
 * Purpose:      resolving options for the Combinatorial Hough Transform (CHT)
 * Date:         Jun 1, 1993
...
char *argv[];
{
  ...
  int c, dim_x, dim_y, dim_rho=ACC_MAX_SIZE, dim_theta=ACC_MAX_SIZE,
      ...
      Shrink=0, NumOfMaxs=999, Segments=8, Overlap=5, MaskSize=2, MinSegLen=10,
      MaxSegWidth=2, MaxSegGap=5, FileFormat=PGM_PIC, StoreMaximas=0;
  double Noise=0.0;
  ...
  while ((c = getopt(argc, argv, "f:G:R:r:F:sn:l:w:g:m:Mihd:t:S:O:W:uP:")) != -1)
    switch (c) {
      ...
      case 't': Threshold=atoi(optarg); 
                break;
      case 'S': Segments=atoi(optarg); 
                break;
      case 'O': Overlap=atoi(optarg); 
                break;
      case 'W': MaskSize=atoi(optarg); 
                break;
  ...
  if (make_object_from_image(File,pic1,Shrink,&dim_x,&dim_y,Noise)) {
    ...
    cht_line(pic,pic1,gpic,rpic,dim_x,dim_y,dim_theta,dim_rho,
             real_params,NumOfMaxs,Threshold,Segments,Overlap,MaskSize,
             MinSegLen,MaxSegWidth,MaxSegGap,TextInfo,ParametersOut);
...
void show_usage()
{
  ...
  fprintf(stderr,"[-d dim_rho dim_theta]");
  fprintf(stderr,"[-t Threshold] [-S Segments] [-O Overlap] [-W MaskSize]\n");
}

/*****************************************************************************/
void print_options()
{
  fprintf(stdout,"Combinatorial Hough Transform (CHT) for lines.\n\n");
  ...
  fprintf(stdout,"\t-S Segments\t\t\t\t\tdef: 8\n");
  fprintf(stdout,"\t   The number of segments in image (Segments^2).\n");
  fprintf(stdout,"\t-O Overlap\t\t\t\t\tdef: 5\n");
  fprintf(stdout,"\t   The number of overlapping pixels in segments.\n");
  fprintf(stdout,"\t-W MaskSize\t\t\t\t\tdef: 2\n");
  fprintf(stdout,"\t   The size of the local maxima search mask ");
  fprintf(stdout,"((2*MaskSize+1)^2).\n");
}
